...
| 维度 | 三交互原语 | 定位 + 感知 + 执行(5 工具) | Bash | Agent | Task 家族 |
|---|---|---|---|---|---|
| 定位 | 协作对齐 | 改代码 | 命令执行 | 派生 Claude | **外化工作记忆** |
| 时态 | 现在时(单次交互) | 现在时(单次操作) | 现在时 | 现在时(fork/join) | **跨时(持久 state)** |
| 状态位置 | 无(靠交互) | 磁盘 + harness | 无(命令结束就没) | subagent 内部 | **runtime 存储** |
| 主要红利 | 用户对齐 | 精准改代码 | 工程流程 | context 空间 | **对抗遗忘 · 协作可见** |
| 命名对偶 | Enter/Exit 对偶 | Read/Edit/Write 同族 | 单一 | 单一 | **CRUD 四件套 + Stop/Output** |

**Task 家族与 Agent 的耦合**是最深的一对:

- Agent 派 subagent 干活 · 结果不可控(可能出错、可能 hang、可能死)
- Task 家族提供**工作项容器** · 让 subagent 的活可以被追踪
- TaskStop 兼容传给 subagent ID 或 task ID · 提供**统一杀灭入口**
- TaskOutput(已废弃)曾是取 subagent 结果的专用接口 · 现在降级为直接 Read subagent 的 output 文件

**Task 家族与 Bash 的类比也值得一说** —— 都是**长时任务的承载**:Bash `run_in_background` 让命令跑在后台,TaskCreate 让 todo 挂在系统里。两者都在解决「AI 主循环阻塞就没法做事」这个问题。区别是:Bash 的后台是「机器等命令返回」,Task 是「人和 AI 协同追进度」 —— 前者是异步 IO,后者是异步协作。

**Task 家族在工具生态里的位置** —— 前 9 个工具都是「一次调用做一件事」的原语,Task 家族是「把要做的事外化到系统里」的元原语。它不是新增了某个能力,而是**为其它所有能力提供了时间维度上的存储**。
...
下一篇继续拆 [WebFetch + WebSearch](../info/web.md) —— 从「文件系统」和「异步任务」跳出来,看 Claude Code 里 AI 与**外部网络**的对话:一个「带 AI 的 curl」,一个「带过滤的搜索」。这两个工具在生态里像姊妹,和 Grep+Glob 的双工具模式对应。
