...
这六个问题，正好对应本书的六个部分：**Tools、Agent Loop、Context、Memory、Skills、MCP**。
...
建议从头开始，依次阅读 Tools → Loop → Context → Memory → Skills → MCP。前四部分的时间尺度逐渐扩大，前面的概念会成为后面的基础；Skills 与 MCP 则回到 Tools 这一层，回答“能力本身怎样继续增长”。
