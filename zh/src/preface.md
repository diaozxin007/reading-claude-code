...
我一直对 [Claude Code](https://www.anthropic.com/claude-code) 的工具设计很感兴趣。表面看,它就是一堆平平无奇的 tool call —— Read / Edit / Bash 之类。但仔细读每个工具的 description 时,会发现里面有大量看似「啰嗦」的约束:
...
这本书就是想拆开这些工具 —— 看看每一个字段名、每一条 prompt 约束、每一次 runtime 硬阻断,分别在防止什么、承载什么、教 Claude 什么。

之前我用 Java 写过一个乞丐版的 Claude Code:[jooj](https://github.com/diaozxin007/jooj)。写完之后对官方版本的很多设计选择有了更深的体会 —— 那些看着「简单」的字段,其实每一个都是精心设计的取舍。这本书某种意义上就是那次 reverse engineering 的产物,只不过用了更容易阅读的形式呈现。
...
**从头读**:章节按分层组织 · 前面章节的概念在后面会被复用。特别是「起步」两章 · 提供了整套系列的心智模型。
...
准备好了 · 从下一章 [Claude Code 的工具设计哲学](philosophy.md) 开始。
