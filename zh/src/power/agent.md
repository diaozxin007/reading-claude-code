...
| 维度 | 三交互原语 | Grep + Glob | Read | Edit / Write | Bash | Agent |
|---|---|---|---|---|---|---|
| 定位 | 协作对齐 | 定位坐标 | 感知外部 | 改文件 | 命令执行 | **派生 Claude** |
| 能力边界 | 有限 · 结构化 | 有限 · 搜索 | 有限 · 读 | 有限 · 写 | 无限 · 真实世界 | **无限 · 递归 Claude** |
| 主要作用 | 与用户对齐 | 定位 | 感知 | 改代码 | 改真实世界 | **压缩信息 · 隔离 context** |
| 通信模型 | 交互式 | 单次调用 | 单次调用 | 单次调用 | 单次调用 | **fork + join(一次性 briefing)** |
| 主要红利 | 用户对齐 | 定位精度 | 感知承诺 | 精准修改 | 工程流程 | **context 空间** |
...
下一篇继续拆 [Task 家族](../state/task-family.md) —— Agent 是「派 subagent 干活」,Task 家族是「管理这些活」。TaskCreate / TaskUpdate / TaskList / TaskGet / TaskStop / TaskOutput 六件套,是 Claude 的「工作记忆外化」,也是整个工具生态里对偶最严整的一族。
