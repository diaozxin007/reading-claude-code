...
| 维度 | 三交互原语 | Grep + Glob | Read | Edit | Write |
|---|---|---|---|---|---|
| 定位 | 协作对齐 | 定位坐标 | 感知外部 | 精准执行 | **全量执行** |
| 频率 | 关键节点 | 日常高频 | 日常高频 | 日常高频 | 中频 |
| 参数 | 结构化 / 空 | pattern | file_path + 分页 | 4 字段(含 old_string) | **2 字段(file_path + content)** |
| 语义 | 意图信号 | 定位坐标 | 感知承诺 | 增量替换 | **全量覆盖 / 新建** |
| 安全网 | 用户批准 | head_limit 截断 | 分页 / PDF 强制页码 | 唯一性 / Read / 匹配失败 | **仅 Read + 父目录存在** |
| 保守偏差 | 「不确定就规划」 | 「先按需搜再全读」 | 「不确定就读一读」 | 「不确定就 Read」 | **「能 Edit 就 Edit · 别新建」** |

**Write 是 Edit 的兄弟工具,不是替代**。二者分工:

- Edit 干**增量修改** —— old_string / new_string / replace_all,基于「文件已存在 + 只改一部分」的假设
- Write 干**新建 / 完全重写** —— content 一次到位,基于「要么没这文件 / 要么整个覆盖」的假设

混着用会失去每个工具的独特安全保障:拿 Write 干 Edit 的活会浪费 tokens + 破坏面失控 + diff 难审阅;拿 Edit 干 Write 的活根本干不了(Edit 不能创建新文件)。

**Grep+Glob → Read → Edit / Write** 四类五个工具共享一套 harness 追踪状态,通过「Read 先行」这条硬约束串联起来。核心哲学:**任何对磁盘的写入,必须建立在对当前磁盘状态的感知之上**。不是靠 AI 自律,而是靠 runtime 强制。
...
下一篇继续拆 [Bash](../power/bash.md) —— 这是整套工具生态里最特殊的一环:**唯一没有边界的兜底工具**。前面 7 个工具都在「让 AI 做有限的事」,而 Bash 让 AI「什么都能做」。看看 Claude Code 团队怎么在「无限能力」和「安全」之间找平衡。
